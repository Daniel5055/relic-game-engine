...
void Framework::Physics::applyDampening(RigidBody* data)
{
    if (abs(data->getVelocity().x) > 0)
    {
        ...
        {
            double x_dampening_force = data->getVelocity().x * data->getVelocity().x * data->getDamping();
    ...
    if (abs(data->getVelocity().y) > 0)
    {
        ...
        {
            double y_dampening_force = data->getVelocity().y * data->getVelocity().y * data->getDamping();
