...
void Jinny::PhysicsSystem::update()
{
    ...
    // Apply damping force to objects
    for (auto body : m_rigid_bodies)
    {
        if (body.second->getDamping() > 0)
        {
            for (int axis : {0, 1})
            {
                ...
                double max_force = abs(body.second->getVelocity()[axis]) * body.second->getMass() / (f_physics->getTimeStep() - tick_time);

                if (max_force > 0)
                {
                    Framework::Vector damping_force = { 0, 0 };

                    damping_force[axis] = body.second->getDamping() * body.second->getMass() / (f_physics->getTimeStep() - tick_time);
    ...
    collision_loop:;
        ...
        for (unsigned int it = 0; it < m_collisions.size(); it++)
        {
            ...
            if (m_collisions[it].rigid_bodies[0]->isStatic() || m_collisions[it].rigid_bodies[1]->isStatic())
            {
                ...
                collision_force[m_collisions[it].axis] = m_col_manager->calculateStaticCollisionForces(dynamic_body->getVelocity()[m_collisions[it].axis],
                    dynamic_body->getAppliedForce()[m_collisions[it].axis], dynamic_body->getMass(),
